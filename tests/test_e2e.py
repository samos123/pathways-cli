...
def test_e2e_pathways_interactive_up_down():
    ...
    assert gcs_location is not None, "PWY_E2E_GCS_SCRATCH_LOCATION env variable must be set in .env"
    ...
    result = runner.invoke(main, [
        "up",
        "--tpu-type", "v6e-4",
        "--gcs-scratch-location", gcs_location,
    ])
    ...
    while time.time() - start_time < timeout:
        ...
        cmd = [
            "kubectl", "get", "pods",
            "-l", "jobset.sigs.k8s.io/jobset-name=pathways-interactive",
            "-o", "jsonpath={.items[?(@.metadata.labels.jobset\\.sigs\\.k8s\\.io/replicatedjob-name==\"pwhd\")].metadata.name}"
        ]
    ...
    pip_proc = subprocess.run([
        "kubectl", "exec", head_pod_name, "-c", "client",
        "--", "pip", "install", "jax", "pathwaysutils"
    ], capture_output=True)
    
    assert pip_proc.returncode == 0, f"pip install failed: {pip_proc.stderr.decode('utf-8')}"
